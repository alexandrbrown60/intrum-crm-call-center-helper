...
$type = $types[1]; //Выьираем второй тип
...
foreach ($list as $key => $item) {
    foreach ($item['fields'] as $fieldKey => $field) {
        ...
        if($list[$key]['fields'][$fieldKey]['id'] == 2126) {
            print_r('<p><input name="property" id="pr-line" value="'.$list[$key]['id'].', '.$list[$key]['author'].'" type="radio"> <a href="https://kluch.intrumnet.com/crm/tools/exec/stock/'.$list[$key]['id'].'#stock" target="_blank">'.$list[$key]['id'].'</a>, '.$list[$key]['fields'][$fieldKey]['value'].'</p>');
        }
    }
    //print_r('<p>'.$list[$key]['id'].', '.$list[$key]['name'].'</p>');
}
